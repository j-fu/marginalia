...
function symbolics(n)
    ...
    t = @elapsed begin
        ...
        SparseDiffTools.forwarddiff_color_jacobian!(jac, A_h!, input,
													colorvec = colors)
...
# ╔═╡ 4c36070c-81a1-4f16-a2c3-1447a615da54
md"""
### DifferentiationInterface
"""

# ╔═╡ 9fa1afed-088c-43d9-9ece-c5bea24e5824
md"""
...
# ╔═╡ 532c9894-a7f5-4447-b426-d33af6b6079e
md"""
Sparsity detection backend from SparseConnectivityTracer.jl:
"""

# ╔═╡ d059f964-d47d-4fb1-8da6-4c77a4c6773e
SCTBackend = AutoSparse(
    AutoForwardDiff();  # any object from ADTypes
    #sparsity_detector = TracerSparsityDetector(),

	sparsity_detector = TracerSparsityDetector(;
    gradient_tracer_type=SparseConnectivityTracer.GradientTracer{
        SparseConnectivityTracer.IndexSetGradientPattern{Int,Set{Int}}
    }
),
    coloring_algorithm = GreedyColoringAlgorithm(NaturalOrder())
)

# ╔═╡ 4f83f090-aa44-411e-86f4-b7092c6acd6c
md"""
Sparsity detection via Symbolics.jl:
"""

# ╔═╡ b054f05a-2dee-49b3-8f88-55acda627b5d
SymbolicsBackend = AutoSparse(
    AutoForwardDiff();  # any object from ADTypes
    sparsity_detector = SymbolicsSparsityDetector(),
    coloring_algorithm = GreedyColoringAlgorithm(IncidenceDegree(), decompression = :direct, postprocessing = true),
)

# ╔═╡ 511188d0-1b21-4734-ad9a-db09f9e7cca9
function di(n, backend)
    input = ones(n)
    output = similar(input)
    tdetect = @elapsed begin
        jac_prep = prepare_jacobian(A_h!, output, backend, input)
        jac = similar(sparsity_pattern(jac_prep), Float64)
    end
    tassemble = @elapsed begin
        jacobian!(A_h!, output, jac, jac_prep, backend, input)
    end
    return jac, tdetect + tassemble
end

# ╔═╡ 8c3b01b9-77db-4118-a55e-b4ebb5544169
di(n_test, SCTBackend)

# ╔═╡ fcca3860-89fe-49dc-a561-16bfd0b976d8
di(n_test, SymbolicsBackend)
...
function A_Jac_h!(y, m, u, flux::F, reaction::R; nrun = 1) where {F, R}
    ...
    for irun in 1:nrun
        ...
        nallocs = @allocated for i in 1:n
            fac = h
            if i == 1||i == n
                fac = 0.5 * h
            end
        ...
        nallocs = @allocated for i in 1:(n - 1)
            UKL[1] = u[i]
            UKL[2] = u[i + 1]
...
The previous approach is at the core of [VoronoiFVM.jl](https://github.com/j-fu/VoronoiFVM.jl), which works for PDE systems on unstructured grids in 1/2/3D.
"""

# ╔═╡ ad4dc34d-78c6-43df-bdea-a7a4bae6f505
vfvm_flux!(f, u, edge, data) = flux!(f, u)

# ╔═╡ 608e1d47-a073-444e-98d1-c00cf57e37cf
vfvm_reaction!(f, u, node, data) = reaction!(f, u)
...
function voronoifvm(n)
    ...
    t = @elapsed begin
        residual, matrix = VoronoiFVM.evaluate_residual_and_jacobian(sys, solution)
    end
    return matrix.cscmatrix, t
end;
...
t_straight = [straightforward(n)[2] for n in N[1:5]]

# ╔═╡ 236988af-339b-442d-a682-b6057b7cbf81
t_symbolic = [symbolics(n)[2] for n in N]

# ╔═╡ 88edcddf-2384-48d5-92b1-d106289d2b00
t_vfvm = [voronoifvm(n)[2] for n in N]

# ╔═╡ 6a63bda5-b48e-4dd5-b616-6f05cc77a620
t_local = [assemblefromlocal(n)[2] for n in N]

# ╔═╡ e22f392d-ae36-4d4b-a5df-0dbebe1e3a79
t_di_sct = [di(n, SCTBackend)[2] for n in N]

# ╔═╡ a017d58d-df14-45c2-bea8-fc5367dd8955
t_di_symbolics = [di(n, SymbolicsBackend)[2] for n in N]

# ╔═╡ fded231c-e478-47bc-a058-d8454c81baa1
let
	fig = CairoMakie.Figure(linewidth=2,size=(650,400))
	ax=Axis(fig[1,1], xscale=log10,yscale=log10, xlabel="n", ylabel="t/s",
		   yticks=LogTicks(LinearTicks(13)))
    scatterlines!(ax,N[1:length(t_straight)], t_straight, label = "ForwardDiff.jacobian!")
    scatterlines!(ax,N, t_symbolic, label = "Symbolics+SparseDiffTools")
    scatterlines!(ax,N, t_di_sct, label = "DI+SparseConnectivityTracer")
    scatterlines!(ax,N, t_di_symbolics, label = "DI+Symbolics")
    scatterlines!(ax,N, t_local, label = "Local jac + ExtendableSparse")
    scatterlines!(ax,N, t_vfvm, label = "Voronoifvm")
    lines!(ax,N, 1.2e-8 * N .^ 2, label = L"O(n^2)", color=:black, linestyle=:dot, linewidth=1)
    lines!(ax,N, 1.2e-10 * N .^ 2, color=:black, linestyle=:dot, linewidth=1)
    lines!(ax,N, 1.0e-6 * N, label = L"O(n)", color=:black, linestyle=:dash, linewidth=1)
    lines!(ax,N, 0.9e-4 * N, color=:black, linestyle=:dash,linewidth=1)
    axislegend(ax, merge=true, position=:lt,backgroundcolor = (:white, 0.75))
    save("sparsejac2.png",fig)
	fig
end
...
function pkgversions(pkgs)
    return Tables.table(reduce(hcat, [[pkg, PkgVersion.Version(pkg) |> string] for pkg in pkgs]) |> permutedims; header = [:package, :version])
end

# ╔═╡ 981e4404-ec77-4d96-9020-23f2ab8963cd
pkgversions([VoronoiFVM, Symbolics, ForwardDiff, SparseDiffTools, DifferentiationInterface, SparseConnectivityTracer, SparseMatrixColorings])
...
SparseDiffTools = "~2.24.0"
SparseMatrixColorings = "~0.4.16"
Symbolics = "~6.38.0"
Tables = "~1.12.0"
VoronoiFVM = "~2.9.1"
"""
...
manifest_format = "2.0"
project_hash = "52c462098f6997b98fc53fe6b7c4eee1d52a4716"
...
git-tree-sha1 = "87d554431d7b2818f67f3a2734c6726787ae69f4"
...
git-tree-sha1 = "dab176139228ee967b6ab388c34184effeb36b64"
...
git-tree-sha1 = "8d0168534bfd1cdcee21bbab42344fb5b06d452d"
